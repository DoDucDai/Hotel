  ...
  useEffect(() => {
    ...
    const submitVerification = async () => {
      ...
      try {
        ...
        setMessage(res?.data?.message || "Email da duoc xac nhan thanh cong");
        toast.success("Xac nhan email thanh cong");
      } catch (error) {
        if (!active) {
          return;
        }
    ...
    <main className="auth-assist-page">
      <section className="auth-assist-shell compact">
        <article className="auth-assist-copy">
          ...
          <h1>Mot buoc nho de kich hoat va xac thuc dia chi email cua ban.</h1>
          <div className="auth-assist-points">
            <article>
              <FaEnvelopeOpenText />
              <div>
                <strong>Xac nhan nhanh</strong>
                <p>Chi can bam vao link trong email de hoan tat xac thuc tai khoan.</p>
              </div>
            </article>
            <article>
              <FaShieldAlt />
              <div>
                ...
                <p>Buoc nay giup dam bao email dang ky la email ban dang thuc su su dung.</p>
              </div>
            </article>
          </div>
        </article>
        ...
        <article className="auth-assist-card">
          ...
          {status === "success" ? (
            <div className="auth-assist-helper success">
              <FaCheckCircle />
              <span>Email cua ban da san sang de su dung cho cac thong bao sau nay.</span>
            </div>
          ) : null}
        </article>
      </section>
